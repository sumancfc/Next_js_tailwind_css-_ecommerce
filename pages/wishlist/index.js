...
export default function WishlistPage() {
    ...
    return  <Layout pageTitle="Wishlist">
    {wishlistItems && wishlistItems.length >= 1 ? (
        <div className="bg-white">
            <div className="container py-10">
                <h1 className="font-medium">Your Wishlist Items ({wishlistItems.length})</h1>
                                    ...
                                    <div className="cart-class">
                                                    ...
                                                    <Image
                                                        src={
                                                            wishlistItem.media.length > 0
                                                                ? `${BACKEND_URL}${wishlistItem.media[0]}`
                                        ...
                                        <div className="w-1/2 hidden cart-details">
                                            ...
                                            <button className="focus:outline-none">
                                                <TrashIcon
                                                    className="h-5 hover:text-main-red"
                                                    onClick={() => deletewishlistItem(wishlistItem)}
                                                />
                                          </button>                                           
                                        </div>
                                    </div>
                ...
                {/* <div className="mt-5 flex flex-col sm:flex-row justify-start sm:justify-between text-center space-y-2 sm:space-y-0"> */}
                    ...
                    <button onClick={() => removeAllFromWishlist()} className="p-3 bg-main-btn text-white text-xs sm:text-sm font-medium tracking-wide focus:outline-none">Clear Wishlist</button>
                </div>
            </div>
        </div>
    ) : (
